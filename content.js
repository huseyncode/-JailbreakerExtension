...
// Function to automatically fill the textarea with the message prompt
function autoText() {
    const textArea = document.querySelector("textarea");
    if (textArea) {
        textArea.value = msgPrompt;
    } else {
        console.error("Textarea not found. The page structure might have changed.");
    }
}

// Function to create and insert the button
function createJailbreakButton() {
    const chatBox = document.querySelector(".flex.flex-col");
    if (chatBox) {
        const button = document.createElement("button");
        button.innerHTML = "JAILBREAK";
        button.style.padding = "5px 10px";
        button.style.borderRadius = "5px";
        button.style.backgroundColor = "#4CAF50";
        button.style.color = "white";
        button.style.margin = "10px 0 10px 10px";
        button.addEventListener("click", autoText);
        chatBox.appendChild(button);
    } else {
        console.error("Chatbox not found. The page structure might have changed.");
    }
}

// Wait for the DOM to fully load before executing the script
window.addEventListener("load", createJailbreakButton);
